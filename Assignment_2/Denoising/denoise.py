# -*- coding: utf-8 -*-
"""
Created on Fri Apr 13 12:30:16 2018
...
for i, img in enumerate(samples):
    img = imread('train/'+str(img))
    ax[i//3, i%3].imshow(img, cmap='gray')
    ax[i//3, i%3].axis('off')
plt.savefig('denoise_samples.png',dpi=300)
plt.close()   
...
for img in train_images:
    img = load_img('train/'+str(img), grayscale=True,target_size=(420,540))
    img = img_to_array(img).astype('float32')/255.
    X.append(img)

for img in train_labels:
    img = load_img('train_cleaned/'+str(img), grayscale=True,target_size=(420,540))
    img = img_to_array(img).astype('float32')/255.
    Y.append(img)
...
sample_test = load_img('test/'+str(test_images[10]), grayscale=True, target_size=(420,540))
...
plt.savefig('denoise_auto.png', dpi=300)
plt.close()  
